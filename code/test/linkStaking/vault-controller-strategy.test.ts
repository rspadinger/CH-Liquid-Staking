...
describe('VaultControllerStrategy', () => {
  ...
  it.only('TEST - depositToVaults should work correctly', async () => {
    ...
    assert.equal(Number((await strategy.globalVaultState())[3]), 0) //depositIndex
    ...
    console.log("+++++++++++++++++ Deposit into vault groups")

    //console.log(await strategy.globalVaultState()) // [ 5n, 0n, 0n, 12n ]
    await fundFlowController.updateVaultGroups()
    //console.log(await strategy.globalVaultState()) // [ 5n, 1n, 0n, 12n ]
    await time.increase(claimPeriod)
    await fundFlowController.updateVaultGroups()
    //console.log(await strategy.globalVaultState()) // [ 5n, 2n, 0n, 12n ]
    await time.increase(claimPeriod)
    await fundFlowController.updateVaultGroups()
    //console.log(await strategy.globalVaultState()) // [ 5n, 3n, 0n, 12n ]
    await time.increase(claimPeriod)
    await fundFlowController.updateVaultGroups()
    //console.log(await strategy.globalVaultState()) // [ 5n, 4n, 0n, 12n ]
    await time.increase(claimPeriod)
    await fundFlowController.updateVaultGroups()
    //console.log(await strategy.globalVaultState()) // [ 5n, 0n, 0n, 12n ]  

    await strategy.withdraw(toEther(50), encodeVaults([0, 5])) //we need to withdraw to create deposit room
    ...
    //console.log(await strategy.globalVaultState()) // [ 5n, 4n, 0n, 12n ]  => num, currUnbond, groupDepInd, depInd
    // console.log("V0", fromEther(await stakingController.getStakerPrincipal(vaults[0]))) //50
    // console.log("V1", fromEther(await stakingController.getStakerPrincipal(vaults[1]))) //0
    // console.log("VG0: ", await strategy.vaultGroups(0)) //V0-5-10 : 0,50
    // console.log("VG1: ", await strategy.vaultGroups(1)) //V1-6-11 : 11, 270
    // console.log("VG2: ", await strategy.vaultGroups(2)) //V2-7-12 : 2, 100
    // console.log("VG3: ", await strategy.vaultGroups(3)) //V3-8-13 : 8, 120
    await strategy.deposit(toEther(50), encodeVaults([0, 1, 6])) //50ETH deposit into V1
    // console.log("V0", fromEther(await stakingController.getStakerPrincipal(vaults[0]))) //50
    // console.log("V1", fromEther(await stakingController.getStakerPrincipal(vaults[1]))) //50
    // console.log("V6", fromEther(await stakingController.getStakerPrincipal(vaults[6]))) //0
    //console.log(await strategy.globalVaultState()) // [ 5n, 4n, 1n, 12n ]  
    
    // console.log("VG0: ", await strategy.vaultGroups(0)) //V0-5-10 : 0,50
    // console.log("VG1: ", await strategy.vaultGroups(1)) //V1-6-11 : 11, 220
    // console.log("VG2: ", await strategy.vaultGroups(2))
    // console.log("VG3: ", await strategy.vaultGroups(3))
  ...
  it('TEST - deposit should work correctly', async () => {
    const { adrs, strategy, token, stakingController, vaults } = await loadFixture(deployFixture)

    await strategy.deposit(toEther(50), encodeVaults([0, 5]))
    assert.equal(fromEther(await token.balanceOf(adrs.stakingController)), 50)
    assert.equal(fromEther(await stakingController.getStakerPrincipal(vaults[0])), 50)
