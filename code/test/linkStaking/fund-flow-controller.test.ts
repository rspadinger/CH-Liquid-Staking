...
describe('FundFlowController', () => {
  ...
  it.only('TEST - getDepositData should work correctly', async () => {
    const { stakingPool, comStrategy, fundFlowController, stakingController, vaults } = await loadFixture(deployFixture)

    await comStrategy.deposit(toEther(1200), encodeVaults([])) //deposits 100 into vault0-11 => GlobalVaultState.depositIndex = 12
    //console.log("Vault Order: ", await fundFlowController.getDepositData(toEther(150)))
    assert.deepEqual(decodeData(await fundFlowController.getDepositData(toEther(150))), [[], []])

    await fundFlowController.updateVaultGroups() //1
    ...
    await fundFlowController.updateVaultGroups()
    await time.increase(claimPeriod)
    await fundFlowController.updateVaultGroups() //0

    //await comStrategy.withdraw(toEther(50), encodeVaults([0, 5])) //withdraw to create depositRoom
    await comStrategy.withdraw(toEther(100), encodeVaults([0, 5])) //withdraw to create depositRoom
    await time.increase(claimPeriod)
    await fundFlowController.updateVaultGroups() //1

    await comStrategy.withdraw(toEther(270), encodeVaults([1, 6, 11])) //0 0 30
    await time.increase(claimPeriod)
    await fundFlowController.updateVaultGroups()
    ...
    await time.increase(claimPeriod)
    await fundFlowController.updateVaultGroups() //4
    await comStrategy.withdraw(toEther(200), encodeVaults([4, 9]))

    console.log("V0", fromEther(await stakingController.getStakerPrincipal(vaults[0]))) //50
    console.log("V1", fromEther(await stakingController.getStakerPrincipal(vaults[1]))) //0
    console.log("V2", fromEther(await stakingController.getStakerPrincipal(vaults[2]))) //0
    console.log("V3", fromEther(await stakingController.getStakerPrincipal(vaults[3]))) //0
    console.log("V4", fromEther(await stakingController.getStakerPrincipal(vaults[4]))) //0
    console.log("V5", fromEther(await stakingController.getStakerPrincipal(vaults[5]))) //100
    console.log("V6", fromEther(await stakingController.getStakerPrincipal(vaults[6]))) //0
    console.log("V7", fromEther(await stakingController.getStakerPrincipal(vaults[7]))) //100
    console.log("V8", fromEther(await stakingController.getStakerPrincipal(vaults[8]))) //80
    console.log("V9", fromEther(await stakingController.getStakerPrincipal(vaults[9]))) //0
    console.log("V10", fromEther(await stakingController.getStakerPrincipal(vaults[10]))) //100
    console.log("V11", fromEther(await stakingController.getStakerPrincipal(vaults[11]))) //30

    console.log("TotDep: ", fromEther(await comStrategy.getTotalDeposits()))
    //console.log("Strat: ", await stakingPool.getStrategies().length)
    // assert.deepEqual(decodeData(await fundFlowController.getDepositData(toEther(150))), [
    //   [],
    //   [0, 1, 6, 11, 4],
    // ])
    //console.log("Vault Order: ", await fundFlowController.getDepositData(toEther(100))) //0,1 => should return [0] 
    //console.log("Vault Order: ", await fundFlowController.getDepositData(toEther(250))) //0,1,6,11,4,9

    //await comStrategy.deposit(toEther(150), encodeVaults([0, 1, 6, 11, 4])) //should return [0, 1, 6]
    //await comStrategy.deposit(toEther(100), encodeVaults([0])) //everything will be deposited into V0
    //await comStrategy.deposit(toEther(250), encodeVaults([0,1,2])) //deposit into V0
  ...
  it('TEST - updateVaultGroups should work correctly', async () => {
