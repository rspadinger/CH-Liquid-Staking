...
describe('OperatorVCS', () => {
  ...
  //TODO: add vaultDepositController to deployFixture return

  it.only('updateDeposits should work correctly', async () => {
    const { accounts, adrs, strategy, stakingPool, rewardsController, token, vaults, vaultDepositController, opVaultImplementation } =
      await loadFixture(deployFixture)


    //upgrade to v3 
    //const OperatorVCSv3 = await ethers.getContractFactory("OperatorVCS");
    // console.log("Strat Address: ", adrs.strategy)
    // console.log("OperatorVCSv3: ", OperatorVCSv3)
    //const upgraded = await upgrades.upgradeProxy(adrs.strategy, OperatorVCSv3, { kind: 'uups', unsafeAllow: ['delegatecall'] });

    // let upgraded
    // try {
    //     upgraded = await upgrades.upgradeProxy(adrs.strategy, OperatorVCSv3, { kind: 'uups', unsafeAllow: ['delegatecall'] });
    //     console.log("Upgrade successful");
    // } catch (error) {
    //     console.error("Upgrade failed:", error);
    // }

    // console.log("OperatorVCS upgraded at proxy address:", adrs.strategy);

    // const newImplementationAddress = await upgrades.erc1967.getImplementationAddress(adrs.strategy);
    // console.log("New implementation address:", newImplementationAddress);
    // console.log("OperatorVCS upgraded to version 3 at proxy:", upgraded.address);

    // await upgraded.initialize(adrs.token,
    //     adrs.stakingPool,
    //     adrs.stakingController,
    //     opVaultImplementation,
    //     [[accounts[4], 500]],
    ...
    console.log("depositIndex", await strategy.globalVaultState())
  ...
  xit('attacker withdraw funds - not working', async () => {
    const { signers, accounts, adrs, strategy, stakingPool, rewardsController, token, vaults, vaultDepositController } =
      await loadFixture(deployFixture)

    let enc = ethers.AbiCoder.defaultAbiCoder().encode(['uint64[]'], [[0,5]])

    await stakingPool.deposit(accounts[0], toEther(1000), [encodeVaults([])])
    
    console.log("VDC: ", vaultDepositController.target)
    console.log("Acc balance before: ",  await token.balanceOf(signers[1]))
    //await stakingPool.withdraw(accounts[0], accounts[0], toEther(130), [encodeVaults([0, 5])])
    //await vaultDepositController.connect(signers[1]).withdraw(1000, enc)    
  })

  xit('TESTold - updateDeposits should work correctly', async () => {
    //Fees: accounts[4] : 5% ; operator reward : 10% ; fees in SP :: accounts[5], 15%,  adrs.erc677Receiver, 20%
    const { stake, accounts, adrs, strategy, stakingPool, rewardsController, token, vaults } =
      await loadFixture(deployFixture)

    //await stake(1, 2000)
    
    await stakingPool.deposit(accounts[0], toEther(400), [encodeVaults([])]) //user sends LINK to SP => deposit into strategy

    //await rewardsController.setReward(comVaults[1], toEther(100))
    await token.transfer(adrs.strategy, toEther(100))

    //console.log("getDepositChange: ", await strategy.getDepositChange()) // 100 reward
    await stakingPool.updateStrategyRewards([0], encode(0)) // we update only for 1 strategy == OperatorVCS

    //console.log("qqq: ", fromEther(await strategy.getTotalDeposits()), fromEther(await strategy.totalPrincipalDeposits()), fromEther(await strategy.getDepositChange()))
    // assert.equal(fromEther(await comStrategy.getTotalDeposits()), 100) //500
    // assert.equal(fromEther(await comStrategy.totalPrincipalDeposits()), 0) //500
    // assert.equal(fromEther(await comStrategy.getDepositChange()), 0)

    //assert.equal(fromEther(await stakingPool.balanceOf(adrs.comStrategy)), 10)
    // assert.equal(fromEther(await stakingPool.balanceOf(accounts[4])), 5)
    // assert.equal(fromEther(await stakingPool.balanceOf(accounts[5])), 15)
    // assert.equal(fromEther(await stakingPool.balanceOf(adrs.erc677Receiver)), 20)
    ...
    // await token.transfer(adrs.strategy, toEther(90)) // like a reward
    // await stakingPool.updateStrategyRewards([0], encode(0))
    // assert.equal(fromEther(await strategy.getTotalDeposits()), 500)
    // assert.equal(fromEther(await strategy.totalPrincipalDeposits()), 400)
    // assert.equal(fromEther(await strategy.getDepositChange()), 0)
    // assert.equal(Number(fromEther(await stakingPool.balanceOf(adrs.strategy)).toFixed(2)), 11.71)
    // assert.equal(Number(fromEther(await stakingPool.balanceOf(accounts[4])).toFixed(2)), 10.36)
