...
describe('StakingPool', () => {
  async function deployFixture() {
    ...
    async function stake(account: number, amount: number) {
      //account first transfers to accounts[0] => accounts[0] then transfers to stakingPool
    ...
    await stakingPool.deposit(accounts[0], 1000, ['0x', '0x']) //owner deposits 1000 wei into SP
  ...
  it.only('TEST - should be able to remove strategies', async () => {
  ...
  it('TEST - should be able to update strategy rewards', async () => {
    const { accounts, adrs, stakingPool, token, strategy, strategy2, erc677Receiver, stake } =
      await loadFixture(deployFixture)

    await stake(1, 2000)
    await stake(2, 1000)
    await stake(3, 2000)
    await token.transfer(adrs.strategy1, toEther(1100)) //reward
   ...
   it('TEST - should be able to update strategy rewards when negative', async () => {
    const { token, signers, accounts, stakingPool, strategy1, stake, withdraw } = await loadFixture(deployFixture)

    console.log(fromEther(await token.balanceOf(accounts[1])))
    await stake(1, 100)
    console.log(fromEther(await token.balanceOf(accounts[1])))
    // await token.connect(signers[1]).transfer(accounts[0], toEther(100))
    // await stakingPool.deposit(accounts[1], toEther(100), ['0x', '0x', '0x'])

    console.log("canWithdraw: ", fromEther(await stakingPool.canWithdraw()))
    await withdraw(1, 80)
    console.log(fromEther(await token.balanceOf(accounts[1])))
    //await stake(2, 1000)

    //await strategy1.simulateSlash(toEther(50))
    //await stakingPool.updateStrategyRewards([0], '0x')
