...
contract OperatorVCS is VaultControllerStrategy {
     ...
     **/
    //reinitializer(3) means this function can only be called once after upgrading the contract to version 3. If the contract is initialized to
    //version 3 (or higher), the function cannot be called again for that version
    function initialize(
        address _token,
        ...
        //this contr inherits from VaultControllerStrategy , which inherits from Strategy => token is defined in strategy
        ...
        if (address(token) == address(0)) {
            ...
            //console.log("Else...");
            //@audit-ok potentially high maxDepositSizeBP hasnt been assigned yet => && depositIndex is wrong
            //uint64 currentDepositIndex = globalVaultState.depositIndex;
            //globalVaultState = GlobalVaultState(5, 0, 0, uint64(currentDepositIndex));
        ...
        //@audit-ok low this should not be required if we already initialized this contract => second case above
        // no where used => probably a low
        //console.log("Push Vaults 2");
     ...
     */
    //any registerd vault (in vaultMapping) can retrieve entire rewards amount
    //called by OpVault::_withdrawRewards
    function withdrawOperatorRewards(
        ...
        //@audit-ok a single vault is allowed to withdraw all rewards ?
        uint256 amountToWithdraw = _amount > withdrawableRewards ? withdrawableRewards : _amount;

        //@audit-ok make sure, this cant underflow
        unclaimedOperatorRewards -= amountToWithdraw;
        //@audit - here, the rewardsReceiver gets LST => in OpVault::_withdrawRewards we send LINK
        ...
        returns (int256 depositChange, address[] memory receivers, uint256[] memory amounts)
    {
        ...
        //iterate all vaults & update vaultDeposits, newTotalPrincipalDeposits, operatorRewards
        for (uint256 i = 0; i < vaultCount; ++i) {
        ...
        console.log("OP-VCS balance: ", balance);
        depositChange = int256(vaultDeposits + balance) - int256(totalDeposits);

        //first receiver is the operator if we have operator rewards
        ...
        if (depositChange > 0) {
            ...
            if (receivers.length == 0) {
                ...
                //operator already received rewards => start with index 1
     ...
     */
    //@audit-ok  we are sure, anyone is allowed to call this func ? => yes, vault has already been removed by CL, so, there is no risk
    //https://github.com/code-423n4/2023-08-chainlink/blob/main/src/pools/OperatorStakingPool.sol
    function queueVaultRemoval(uint256 _index) external {
        ...
        if (_index < globalVaultState.depositIndex) {
            ...
            groups[0] = group;
            //@audit-ok make sure, this func takes into account that the vault has been removed => does it check the vaultsToRemove arr ?
            fundFlowController.updateOperatorVaultGroupAccounting(groups);

            // if possiible, remove vault right away
            //@audit-ok can we remove vault if claimPeriodActive or shold there be a !
            //@audit-ok here we check if claimPeriodActive => why isnt this checked in the removeVault func? => an attacker can call remove immed after queue =>
            //this won't allow him to execute the removeVault func if !claimPeriodActive
     ...
     */
    //@audit-ok can anyone really remove any vault in the queue
    //@audit-ok should check: if (vaults[_index].claimPeriodActive()) - check if this fails if an attacker calls this func & !claimPeriodActive =>
    // i guess the following will fail in exitVault() : stakeController.unstakeRemovedPrincipal();
    ...
    function updateVaultGroupAccounting(
        ...
        //console.log("updateVaultGroupAccounting");
