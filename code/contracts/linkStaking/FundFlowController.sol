...
contract FundFlowController is UUPSUpgradeable, OwnableUpgradeable {
     ...
     */
    //when LINK should be deposited into CL pools, this func will be called to generate deposit data =>
    //passed to PP => passed to strategy contracts to decide which vaults should be deposited into => this is done to save gas
    //the getWithdrawalData is doing the same for withdrawals
    function getDepositData(uint256 _toDeposit) external view returns (bytes[] memory) {
        //@audit-ok why *2 ? => _toDeposit is per strategy & we have 2 strats
        //first deposit into opVCS, then into ComVCS
     ...
     * current vault group
     */
    //In CL, LINK is staked indefinitely => unbond needs to be initialized, which lasts 28 days => then, there is a 7 day claim period
    //we have 5 vault groups => one is always unbonded => iterated every 7 days (by calling this function)
    //@audit-ok anyone can call this ? => probably not a problem
    function updateVaultGroups() external {
        ...
        // claim period must be concluded (expired) for current group
        if (
        ...
        if (
            curUnbondedGroup != 0 && // 1
            timeOfLastUpdateByGroup[curUnbondedGroup] == 0 &&
            block.timestamp <= timeOfLastUpdateByGroup[curUnbondedGroup - 1] + claimPeriod
            //@audit-ok shouldnt we add unbondingPeriod here ?
            //after deployment, we need to call updateVaultGroups() to set : timeOfLastUpdateByGroup[curUnbondedGroup] = NOW
            //if we call updateVaultGroups() immediately afterwards => curUnbondedGroup = 1 & nextUnbondedGroup = 2
        ) {
            console.log(
                "NO UPDATE: ",
                curUnbondedGroup,
                timeOfLastUpdateByGroup[curUnbondedGroup],
                timeOfLastUpdateByGroup[curUnbondedGroup - 1]
            );
            revert NoUpdateNeeded();
        }
    ...
    function _getVaultDepositOrder(
        ...
        // add the remaining depositRoom from the vault we left off
        console.log("GDI: ", groupDepositIndex); //0

        if (groupDepositIndex < maxVaultIndex) {
            //@audit-ok dont perform those actions if the condition below is not met => nothing will be deposited into this vault
            //in VCS::deposit => vaultIds[0] != globalState.groupDepositIndex => revert!!!
            vaultDepositOrder[0] = groupDepositIndex;
            ++totalVaultsAdded;

            (uint256 withdrawalIndex, ) = _vcs.vaultGroups(groupDepositIndex % numVaultGroups); //get the corresp vaultGroup
        ...
        //console.log("TD1: ", _toDeposit, totalDepositsAdded); //300, 0

        // iterate through groups in group deposit order filling each vault and group entirely before moving onto the next
        //@audit-ok !!! add condition before loop: if (totalDepositsAdded < _toDeposit)
        //without this, we may add an additional vault to vaultDepositOrder & add a not required value to totalVaultsAdded - is this a problem?
        for (uint256 i = 0; i < numVaultGroups; ++i) {
            //VaultGroup[] public vaultGroups;
            (uint256 withdrawalIndex, ) = _vcs.vaultGroups(groupDepositOrder[i]); // groupDepositOrder[i] == index of group from largest to smallest

            //iterate all vaults inn the current group
            //groupDepositOrder[i] == 0..4 => starting from the group with highest depositRoom
            for (uint256 j = groupDepositOrder[i]; j < maxVaultIndex; j += numVaultGroups) {
                uint256 deposits = IVault(vaults[j]).getPrincipalDeposits();

                // console.log("IND: ", j, maxVaultIndex); /0-12 , 5-12, 10-12, 1-12...
                //console.log("IND: ", deposits, maxDeposits); //100 100
                if (j != groupDepositIndex && deposits != maxDeposits) {
                    ...
                    //@audit-ok why do we count deposit room if deposits == 0 ? => this is ok : if deposits == 0, nothing can be withdrawn
                    if (j != withdrawalIndex || deposits == 0)
                        totalDepositsAdded += maxDeposits - deposits;

                    //console.log("TD1: ", _toDeposit, totalDepositsAdded);
    ...
    function _getTotalDepositRoom(
        ...
        //iterate all vaults in the specified vaultGroup
     ...
     */
    //unbonded means, claimPeriod must be active for the vault
