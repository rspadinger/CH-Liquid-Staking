...
contract OperatorVault is Vault {
    ...
    function initialize(
        ...
        if (vaultController == address(0)) {
            ...
            //@audit-ok we assume, vaultController & token is already set ?
     ...
     */
    //called by rebaseContr or by strategy on addFee & updateFee
    function updateDeposits(
        uint256 _minRewards,
        address _rewardsReceiver
    ) external onlyVaultController returns (uint256, uint256, uint256) {
        uint256 principal = getPrincipalDeposits(); // principal balance of this Vault in the Chainlink staking contract
        uint256 rewards = getRewards(); //get total rew amount from stake controller - claimable rewards balance from CL
        uint256 totalDeposits = principal + rewards;

        //trackedTotalDeposits initally set to: getPrincipalDeposits() + getRewards(); =>
        //depositChange == new claimable rewards (not yey tracked)
        ...
        //@audit-ok if this func is called several times && rewards >= _minRewards => the vars above will be falsely updated several times
    ...
    function _withdrawRewards() private {
        uint256 rewards = getUnclaimedRewards(); // unclaimedRewards + token.balanceOf(address(this)) //300
        uint256 balance = token.balanceOf(address(this)); //200

        //send amountWithdrawn to operator (rewards)
        uint256 amountWithdrawn = IOperatorVCS(vaultController).withdrawOperatorRewards(
            rewardsReceiver,
            rewards - balance //100
        ); // => 100 sent to rewardsReceiver

        unclaimedRewards -= SafeCast.toUint128(amountWithdrawn);

        //@audit-ok why do we also send the balance to the rewardsReceiver => we also send the remaining balance
        //here, we send LINK to rewardsReceiver => token.balanceOf(address(this))
        if (balance != 0) {
            token.safeTransfer(rewardsReceiver, balance); //200 sent to rewardsReceiver
        }
