...
contract PriorityPool is UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
     ...
     */
    //@audit-info  If you need to return a large dataset, it's often better to implement pagination =>
    //When returning large arrays, the data must be encoded and large datasets can lead to problems (response time & memory limits) in client apps
    ...
    function getQueuedTokens(
        address _account,
        uint256 _distributionAmount //distribution of LST
    ) public view returns (uint256) {
        //@audit-ok is this the currently queued amount => is this mapping updated on deposits/unqueue ?
        //whenever there is a distribution, those tokens are no longer considered to be queued?
        return accountQueuedTokens[_account] - _distributionAmount; //totalQ = queued LINK - distribution of LST
    }
    ...
    function getLSDTokens(
        ...
        //@audit-ok check values: is _distributionShareAmount == all time distrib
        //accountSharesClaimed is updated each time claimLSDTokens is called
    ...
    function canWithdraw(
        ...
        //canUnqueue = #User LINK queued in PP
        //stLINKCanWithdraw = #User LST in SP
        //total = canUnqueue + stLINKCanWithdraw = canUnqueue + stakingPool.canWithdraw() + totalQueued - canUnqueue = stakingPool.canWithdraw() + totalQueued

        //@audit-ok verify: stakingPool.canWithdraw() + totalQueued - canUnqueue => why: totalQueued - canUnqueue
        //should be fine: we take TQ, but we need to subtract the previously calculated CU
     ...
     **/
    //call transferAndCall on LINK to send LINK to PP => _sender == msg.sender of transferAndCall
    //user/contr calls transferAndCall (eg on LINK) to send tokens => onTokenTransfer is called on receiver contr : forwards msg.sender
    function onTokenTransfer(address _sender, uint256 _value, bytes calldata _calldata) external {
        ...
        //@audit-ok when is this called - on all token transfers? - can anyone send LINK & specify _sender... , calldata => -sender is always me
        //function transferAndCall(address to, uint256 value...
        //can we manipulate calldata => transfer LINK to this contract & specify bad calldata => check what happens with data
        //TokenReceiver(to).onTokenTransfer(msg.sender, value, data);
        // msg.sender == contract that calls transferAndCall on LINK contract => calls onTokenTransfer on target contract
        ...
        if (msg.sender == address(token)) {
            ...
            //SP sends LST
            // stakingPool sends LST => we need to withdraw LINK
            uint256 amountQueued = _withdraw(_sender, _value, shouldQueue); // amount of tokens that were queued for withdrawal =>
            //will be 0 if everything is avail for withdrawal

            //_value - amountQueued == amount that is available & can be sent immediately
            //@audit-ok could there be a scenario, where this fails, because there arent enough tokens to send
    ...
    function deposit(uint256 _amount, bool _shouldQueue, bytes[] calldata _data) external {
        ...
        //@audit-ok can user manipulate _data
        //calls _depositToVaults in VaultDepositController => [vaultIds] => attacker could provide any vault - is this a problem?
    ...
    function withdraw(
        ...
        if (_shouldUnqueue == true) {
            ...
            uint256 queuedTokens = getQueuedTokens(account, _amount); //account's current amount of deposits in the pool
        ...
        if (toWithdraw != 0) {
            //acc transfers LST to PP & receives LINK (below)
            IERC20Upgradeable(address(stakingPool)).safeTransferFrom(
                account,
                address(this),
                toWithdraw
            ); // user transfers LST back to priority pool

            //return the amount of tokens that were queued for withdrawal == 0 if nothing needs to be queued & everything is avail for withdrawal
            //this reverts if _shouldQueueWithdrawal = false  & not all tokens are available
            toWithdraw = _withdraw(account, toWithdraw, _shouldQueueWithdrawal);
        }

        token.safeTransfer(account, _amountToWithdraw - toWithdraw); //transfer LINK to user
    }
     ...
     */
    //@audit-ok chack if unqueuing happens in correct order for accounts => eg: we are queuing in _deposit
    //anyone can unqueue as long as there are enough tokens queued
    function unqueueTokens(
        ...
        //if acc is not in mapping, this is 0 & merkleTreeSize => verify will fail
        ...
        //getQueuedTokens : # total tokens user has queued => cannot withdraw more
     ...
     * @param _merkleProof merkle proof for sender's merkle tree entry (generated from IPFS data)
     */
    // _amount == what is currently claimable
    function claimLSDTokens(
        ...
        //@audit-ok this means, we can withdraw the entire amount (as stored on IPFS) => who modifies the _amount value on IPFS? =>
        // see description on: getAccountData()
        //this value must be updated periodically
    ...
    function performUpkeep(bytes calldata _performData) external {
        ...
        // console.log("DD Length: ", depositData.length);
        //console.logBytes(depositData[0]);
        ...
        returns (address[] memory, uint256[] memory, uint256[] memory)
    {
        ...
        //@audit-ok out of bounds if called from state changing func => all ccounts that ever queued tokens
    ...
    function updateDistribution(
        ...
        //deposit of LST => incr in withdraw func
        //here, LST is distributed to users => afterwards, user can call claim
    ...
    function executeQueuedWithdrawals(
        ...
        IERC20Upgradeable(address(stakingPool)).safeTransferFrom(
            ...
            address(this),
            _amount
        ); //WithdrawalPool sends LST to PriorityPool

        stakingPool.withdraw(address(this), address(this), _amount, _data); //execute the withdrawal on SP => send LINK to PP
        token.safeTransfer(msg.sender, _amount); //send LINK to WithdrawalPool
    }
    ...
    //called when user sends LINK via onTokenTransfer => gets LST
    function _deposit(
        address _account,
        uint256 _amount,
        bool _shouldQueue,
        ...
        //nothing queued in PP  & queuedWithdrawals in WP
        //in WP someone provides LST to get LINK => we can take that LST
        if (totalQueued == 0) {
            uint256 queuedWithdrawals = withdrawalPool.getTotalQueuedWithdrawals(); //withdraw LINK
            if (queuedWithdrawals != 0) {
                ...
                //transfer LST to account from PP after receiving LST from WP
                IERC20Upgradeable(address(stakingPool)).safeTransfer(_account, toDepositIntoQueue);
            }

            //are there still tokens to deposit? => check if we can directly deposit LINK into SP => LST is directly minted to _account
            if (toDeposit != 0) {
        ...
        //if there is still some leftover to deposit => queue LINK
        if (toDeposit != 0) {
            if (_shouldQueue) {
                _requireNotPaused();

                //queue for later deposit into SP => check if account already deposited previously
                ...
                token.safeTransfer(_account, toDeposit); //transfer remaining LINK back to account
            }
        }
    ...
    function _withdraw(
        ...
        if (totalQueued != 0) {
            ...
            //acc will transfer LST in exchange for LINK => increase deposits
        ...
        //this LINK amount will be transferred to account
        return toWithdraw;
    }
     ...
     * @param _data deposit data passed to staking pool strategies
     **/
    //called by performUpkeep & depositQueuedTokens
    function _depositQueuedTokens(
        ...
        //this also handles deposit of unused tokens in SP => toDepositFromStakingPool
        stakingPool.deposit(address(this), toDepositFromQueue, _data);
        _totalQueued -= toDepositFromQueue;

        //tokens we could not deposit
