...
contract StakingPool is StakingRewardsPool {
    ...
    function initialize(
        ...
        //@audit-ok no hardcode, magic number => needs to be verified when we add new fees
    ...
    function deposit(
        ...
        //@audit-ok test for amount = 1 wei => can we mint?
        if (_amount > 0) {
            token.safeTransferFrom(msg.sender, address(this), _amount);
            _depositLiquidity(_data);
            //@audit-ok - can we immediately mint all LSTs ? yes, whatever amount of LINK a user is allowed to deposit into SP is sent back to user as LST
     ...
     **/
    //called by PP::executeQueuedWithdrawals (pp,pp...)
    function withdraw(
        address _account,
        address _receiver,
        uint256 _amount,
        ...
        //@audit-ok test for amount = 1 wei - can we burn
        uint256 toWithdraw = _amount;
        if (_amount == type(uint256).max) {
            toWithdraw = balanceOf(_account); //LST balance for user
        }

        //@audit-ok do we also need to check: getMinDeposits() => min amount tokens that need to stay in pool - we are withdrawing LINK not LST
        //should be fine: in _withdraw(), we check on each strategy: strategy.canWithdraw();

        uint256 balance = token.balanceOf(address(this)); //LINK balance of this contract
        if (toWithdraw > balance) {
            //not enough left, withdraw from strategy
            _withdrawLiquidity(toWithdraw - balance, _data);
        }

        //check new balance after strat withdrawal
        require(
            token.balanceOf(address(this)) >= toWithdraw,
            "Not enough liquidity available to withdraw"
        );

        _burn(_account, toWithdraw); //fails if user does not have enough LST
    ...
    function strategyDeposit(
        ...
        //@audit-ok should check canDeposit: strategyCanDeposit = strategy.canDeposit();...  if (strategyCanDeposit >= toDeposit) {
        // => verified in VCS::_depositToVaults
        //should also check avail funds: uint256 toDeposit = token.balanceOf(address(this));
        //what if data is wrong? => data is checked
        //check if this fails if funds not avail and/or strategy room too small
    ...
    function strategyWithdraw(
        ...
        //@audit-ok as above: check if this fails if _amount too large
    ...
    function getMinDeposits() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < strategies.length; i++) {
            ...
            //@audit-ok == total tokens staked in strategy (totalDeposits) ??? => VaultControllerStrategy::getMinDeposits
    ...
    function canWithdraw() external view returns (uint256) {
        //@audit-ok corresp with totalDeposits in all strategies together ??? => called by PriorityPool
        //totalStaked : sum of all deposited & donated tokens
    ...
    function removeStrategy(
        ...
        //console.log("Length: ", strategies.length);
        require(_index < strategies.length, "Strategy does not exist");

        uint256[] memory idxs = new uint256[](1);
        idxs[0] = _index;

        //@audit-ok check this => can this fail...
        ...
        //@audit-ok attention: as long as there are deposits, the strategy cant be withdrawn - what happens with vaults after strat removal - cant be removed?
        //@audit-ok after exitVault (in OperatorVault), the vault funds are sent to the VCS => how to get those funds out => OperatorVCS::removeVault => sent to SP
        if (totalStrategyDeposits > 0) {
            //console.log("222");
            //remove vault from CL => queueVaultRemoval => remove vault => exit vault
            //@audit-ok !!! are we sure, everything will be withdrawn (ComVault) - could there be a scenario, where withdrawal constantly fails
            //and prevents strategy from being removed .
    ...
    function getStrategyRewards(
        ...
        //@audit-ok totalRewards should be: totalRewards - totalFees => check how totalFees are calculated =>
        //do the returned totalRewards already factor them in?
        ...
        //@audit-ok should be: totalFees = totalStaked
    ...
    function updateStrategyRewards(uint256[] memory _strategyIdxs, bytes memory _data) external {
        //call either by rebaseController or strategy
    ...
    function _depositLiquidity(bytes[] calldata _data) private {
        //@audit-ok what is _data ? => vaultIds to deposit into
        uint256 toDeposit = token.balanceOf(address(this));
        if (toDeposit > 0) {
            // console.log("Strat Length: ", strategies.length);
            // console.logBytes(_data[0]);
            for (uint256 i = 0; i < strategies.length; i++) {
                IStrategy strategy = IStrategy(strategies[i]);
     ...
     * @param _data update data passed to each strategy
     **/
    //is only called by rebaseController or strategy - or: Owner if removeStrategy is called
    //@audit-ok complex - check again
    //VCS periodically calls claimRewards on all vaults => accumulated rexards from CL are sent to VCS => token.balanceOf(address(this)); =>
    // token.safeTransfer(address(stakingPool), balance); => SP calls updateStrategyRewards & receives that balance
    // the only token balance we find on a strategy is always from rewards
    function _updateStrategyRewards(uint256[] memory _strategyIdxs, bytes memory _data) private {
        //@audit-ok make sure: _strategyIdxs.length <= strategies.length

        //console.log("Initial bal: ", balanceOf(address(this)));

        int256 totalRewards;
        uint256 totalFeeAmounts;
        uint256 totalFeeCount; // number of fee receivers : strategy fees + fees of staking pool

        //arr size: one dyn arr for each strategy + 1 dyn arr for fee receivers defined in this contr
        ...
        for (uint256 i = 0; i < _strategyIdxs.length; ++i) {
            IStrategy strategy = IStrategy(strategies[_strategyIdxs[i]]);
            //console.log("Strat Total Deposit: ", i, strategy.getTotalDeposits());

            (
                int256 depositChange,
                address[] memory strategyReceivers,
                uint256[] memory strategyFeeAmounts
            ) = strategy.updateDeposits(_data); //entire balance from all strateigies (VCS) is sent to SP - data == minRewards

            //console.log("Dep change: ");
            //console.logInt(depositChange);
        ...
        //up to here: totalFeeCount = all strat receivers ; totalFeeAmounts = sum(amounts) all strat receivers ; totalRewards = total deposit changes
        // console.logInt(totalRewards);
        // console.log("TS: ", totalStaked);

        // update totalStaked if there was a net change in deposits
        if (totalRewards != 0) {
            //@audit-ok all penalties (totalRewards) would need to be higher than totalStaked => this is not possible
            //could int256(totalStaked) + totalRewards be negative => rewards can be neg because of slashing
            //casting a negative int256 to uint256 returns a huge positive value => uint(-1) = type(uint256).max
            //however, it is unprob that totalStaked < totalRewards => maybe at beginning ?
            //if negative => set totalStaked = 0
            totalStaked = uint256(int256(totalStaked) + totalRewards);
            //console.log("TS: ", totalStaked);
        }
        // IStrategy strategy1 = IStrategy(strategies[_strategyIdxs[0]]);
        // console.log("Strat Total Deposit: ", strategy1.getTotalDeposits());

        // calulate fees if net positive rewards were earned
        //depositChange = contains only deposit changes (no fees are added/deducted)
        //@audit-ok shouldnt we deduct the strategy fees (above => current totalFeeAmounts amount) from the totalRewards, before calculating the feeAmounts below
        //no, the provided % are in terms of totalRewards & can be 70% max all together - total fees all strategies: max 30% ; total fees staking pool: max 40%
        if (totalRewards > 0) {
            //receivers.length - 1 => this is the last element => for the staking pool fee receivers
        ...
        if (totalFeeAmounts >= totalStaked) {
            //@audit-ok rewards are always lower than totalStaked & fees are only a % of rewards => we should never get a value that high
            //if it really would happen, we reset the fees to 0.  shouldnt this be: totalFeeAmounts = totalStaked
            totalFeeAmounts = 0;
        }

        // distribute fees to receivers if there are any
        if (totalFeeAmounts > 0) {
            //we need to mint #Shares that corresp with totalFeeAmounts => currently, the TotalShares corresp with : totalStaked - totalFeeAmounts
            //@audit-ok is this correct => totalFeeAmounts is leaving the system => deduct it from totalStaked =>
            //sharesToMint == corresp shares amount of totalFeeAmounts
            // totalStaked - totalFeeAmounts == totalShares (100%) && totalFeeAmounts == sharesToMint

            //??? @audit-ok #shares to mint needs to corresp with totalFeeAmounts
            //sharesToMint is wrong => replace totalFeeAmounts with totalRewardsReceived - totalFeeAmounts
            //our totalStaked value already contains all rewards => but, fees will be paid out (leave the system), so, they need to be deducted from
            //totalStaked to get the updated/correct version of totalStaked => this value now corresponds with the totalShares in the SP
            //now, we need to calculate the corresp share value for the rewardReceived: balanceReceived - totalFeeAmounts =>
            //SharesRewardReceived = rewardReceived * totalShares / (totalStaked - totalFeeAmounts)

            //OR :: does the SP get shares for the amount of paid out fees ???
            uint256 sharesToMint = (totalFeeAmounts * totalShares) /
                (totalStaked - totalFeeAmounts);

            //we added to totalStaked : totalRewards (contain all fees) => we need to mint #Shares that corresp with those added totalRewards
            //when updateDeposits is called on each strategy, depositChange is the exact value

            //mint LST tokens => minted shares must corresp with received deposit from all strategies
            //we received a certain amount of tokens (LINK) from various VCS (all rewards - not only the fees!)
            //we need to mint the prop amount of shares
            //this just modified 2 SVs: totalShares += _amount; &&  shares[_recipient] += _amount;
            //console.log("Curr bal: ", sharesToMint, balanceOf(address(this)));
            _mintShares(address(this), sharesToMint);

            //console.log("Curr bal: ", sharesToMint, balanceOf(address(this))); //we mint #feesToBePaid token

            //distribute fees
            uint256 feesPaidCount;
            // receivers.length corresp with #strategies
            for (uint256 i = 0; i < receivers.length; i++) {
                for (uint256 j = 0; j < receivers[i].length; j++) {
                    //console.log("feesPaidCount: ", feesPaidCount);
                    //@audit-ok cross check if this cant be messed up
                    if (feesPaidCount == totalFeeCount - 1) {
                        //console.log("final bal: ", balanceOf(address(this)));
                        //console.log("Fee: ", feesPaidCount, feeAmounts[i][j]);
                        //@audit-ok => here, we handle the LST tokens we minted above (corresp with calculated total fees)
                        //we get rewards from the VCS (contains rewards+fees) => in our loop we pay all fees (maybe 40% of rewards)
                        //BUT, the last fee receiver gets everything that is left => thats wrong !
                        // shares to be transferred = (feeAmounts[i][j] * totalShares) / (totalStaked - feeAmounts[i][j])
                        transferAndCallFrom(
                            address(this),
                            receivers[i][j],
                        ...
                        //@audit-ok here we transfer feeAmounts[i][j] shares => but, the minted shares don't correspond 1:1
                        //shouldnt we send the corresp share amounts, calculated from feeAmounts[i][j]
                        // shares to be transferred = (feeAmounts[i][j] * totalShares) / (totalStaked - feeAmounts[i][j])
