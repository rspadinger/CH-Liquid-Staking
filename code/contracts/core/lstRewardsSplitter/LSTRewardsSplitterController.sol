...
contract LSTRewardsSplitterController is Ownable {
     ...
     **/
    //Splitter deposits LST to the controller
    //any of the spitter contracts can call transferAndCall on LST to send LST to this controller contr
    ...
    function removeSplitter(address _account) external onlyOwner {
        ...
        console.log("Old Bal: ", balance); //200
        uint256 principalDeposits = splitter.principalDeposits();
        console.log("PD: ", principalDeposits); //100

        if (balance != 0) {
            if (balance != principalDeposits) splitter.splitRewards();
            //@audit-ok the bal retrieved above is no longer available (we deducted rewards distrib)
            ...
            console.log("New Bal: ", newBalance); //170 after reward split

            //splitter.withdraw(newBalance, _account);
            splitter.withdraw(balance, _account);
            //this would fail: principalDeposits -= balance;
        }
