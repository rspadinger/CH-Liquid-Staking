...
contract LSTRewardsSplitter is Ownable {
    ...
    function checkUpkeep(bytes calldata) external view returns (bool, bytes memory) {
        ...
        //we can have either a loss or rewards above the threshold
    ...
    function performUpkeep(bytes calldata) external {
        ...
        if (newRewards < 0) {
            //update principalDeposits to reflect losses
    ...
    function splitRewards() external {
        ...
        if (newRewards < 0) {
            ...
            revert InsufficientRewards(); //we bypasses rewardThreshold
        } else {
            _splitRewards(uint256(newRewards));
        }
    }
    ...
    function addFee(address _receiver, uint256 _feeBasisPoints) external onlyOwner {
        //@audit-ok  check if receiver already exists => same receiver could be added twice
    ...
    function _splitRewards(uint256 _rewardsAmount) private {
        ...
        //console.log("BAL: ", lst.balanceOf(address(this)));

        principalDeposits = lst.balanceOf(address(this)); //old balance - distributed rewards
